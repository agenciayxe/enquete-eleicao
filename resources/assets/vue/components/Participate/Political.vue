...
class Political extends Vue {
    ...
    @Prop() getSurvey;
    audio = new Audio("/audio/urna.mp3");  
    ...
    answers(question) {        
        const votes = this.votes.map((vote) => this.stripHTML(vote.body));
        return question.answers.filter((answer) => !votes.includes(this.stripHTML(answer.body)));
    }
    ...
    next() {     
        ...
        else this.step = this.survey.questions.length - 1;
    }
    ...
    setSelected(answer, index) {
        if(index) {
            ...
            votes[index] = { answer_id: answer.id, question_id: answer.question_id, body: answer.body, justify: answer.justify };        
            this.votes = votes;
            return;
        }
        const votes = [...this.votes];
        votes[this.step] = { answer_id: answer.id, question_id: answer.question_id, body: answer.body, justify: answer.justify };        
        this.votes = votes;
        this.audio.play();
        if(this.hasNext) this.next();
        if(this.surveyComplete) this.vote();
    }
                                ...
                                b-list-group
                                    ...
                                    b-list-group-item(class="d-flex justify-content-between align-items-center" v-for="answer in question.answers" :key="answer.id")
                                        ...
                                        span 
                                            ...
                                            |  
                                            span(:style="`left: ${!!answer.image ? 50 : 0 }px; position: relative;`") {{ stripHTML(answer.body) }}
                        ...
                        b-col(v-else)
                            ...
                            template(v-else-if="!voted && question")                                
                                b-card
                                    ...
                                    br
                                    b-card(
                                        v-for="answer in answers(question)"
